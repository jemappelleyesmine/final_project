...
### Age
```{r}
# Combine age groups into 4 broader categories
melted_data <- new_dataset %>%
  select(
    age_15_24_value,
    age_25_34_value,
    age_35_44_value,
  ...
  mutate(
    Age_Group = case_when(
      Age_Group %in% c("age_15_24_value", "age_25_34_value") ~ "15-34",
      Age_Group %in% c("age_35_44_value", "age_45_54_value") ~ "35-54",
      Age_Group %in% c("age_55_64_value", "age_65_74_value") ~ "55-74",
      Age_Group %in% c("age_75_84_value", "age_85_94_value") ~ "75-94",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(Age_Group)) %>% # Remove NA Age_Group values
  pivot_longer(
...
# Summarize transportation choices by broader age groups
summary_data <- melted_data %>%
  group_by(Age_Group, Transport_Mode) %>%
  summarise(Average_Value = mean(Transport_Value, na.rm = TRUE), .groups = "drop")

# Create the faceted bar plot
ggplot(summary_data, aes(x = Age_Group, y = Average_Value, fill = Age_Group)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Transportation Choices by Broad Age Groups",
    x = "Broad Age Group",
    y = "Average Value"
  ) +
  theme_minimal() +
  theme(legend.position = "none") + # Remove legend as Age Group is on x-axis
...
```{r}
# Melt the data for transportation modes grouped by gender
melted_data_gender <- new_dataset %>%
  select(
    male_value,
    female_value,
    no_transport_value,
  ...
  pivot_longer(
    cols = c(male_value, female_value),
    names_to = "Gender",
  ...
  mutate(
    Gender = case_when(
      Gender == "male_value" ~ "Male",
      Gender == "female_value" ~ "Female",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(Gender)) # Remove NA Gender values

# Summarize transportation choices by gender
summary_data_gender <- melted_data_gender %>%
  group_by(Gender, Transport_Mode) %>%
  summarise(Average_Value = mean(Transport_Value, na.rm = TRUE), .groups = "drop")

# Create the faceted bar plot
ggplot(summary_data_gender, aes(x = Gender, y = Average_Value, fill = Gender)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Transportation Choices by Gender",
    x = "Gender",
    y = "Average Value"
  ) +
  theme_minimal() +
  theme(legend.position = "none") + # Remove legend as Gender is on x-axis
  facet_wrap(~ Transport_Mode, scales = "free_y") # Facet by Transport Mode
...
```{r}
# Melt the data for commuting distances
melted_data_distance <- final_dataset_cleaned2.0 %>%
  select(
    within_residence_commune_share,
    other_commune_same_dept_share,
    other_dept_same_region_share,
    outside_region_metropole_share,
    outside_region_dom_share,
    outside_region_com_share,
    abroad_share,
    no_transport_share,
    on_foot_share,
    bicycle_share,
  ...
  pivot_longer(
    cols = c(no_transport_share, on_foot_share, bicycle_share, 
             ...
             public_transport_share),
    names_to = "Transport_Mode",
    values_to = "Transport_Share"
  ) %>%
  mutate(
    Commuting_Distance = case_when(
      Commuting_Distance == "within_residence_commune_share" ~ "Within Residence Commune",
      Commuting_Distance == "other_commune_same_dept_share" ~ "Other Commune Same Dept",
      Commuting_Distance == "other_dept_same_region_share" ~ "Other Dept Same Region",
      Commuting_Distance == "outside_region_metropole_share" ~ "Outside Region Metropole",
      Commuting_Distance == "outside_region_dom_share" ~ "Outside Region DOM",
      Commuting_Distance == "outside_region_com_share" ~ "Outside Region COM",
      Commuting_Distance == "abroad_share" ~ "Abroad",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(Commuting_Distance)) # Remove rows with NA categories

# Summarize transportation choices by commuting distances
summary_data_distance <- melted_data_distance %>%
  group_by(Commuting_Distance, Transport_Mode) %>%
  summarise(Average_Share = mean(Transport_Share, na.rm = TRUE), .groups = "drop")

# Create the faceted bar plot
ggplot(summary_data_distance, aes(x = Commuting_Distance, y = Average_Share, fill = Commuting_Distance)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Transportation Choices by Commuting Distance Categories",
    x = "Commuting Distance Category",
    y = "Average Share"
  ) +
  theme_minimal() +
  theme(
