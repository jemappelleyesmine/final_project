...
local_dir <- "data"
csv1_name <- "FD_MOBPRO_2021.csv.gz" # Compressed file name
csv2_name <- "varmod_mobpro_2021.csv.gz" # Compressed file name
...
# Define original uncompressed CSV file names from the ZIP
original_csv1 <- "FD_MOBPRO_2021.csv"
original_csv2 <- "varmod_mobpro_2021.csv"

# Check and process the first CSV
if (!file.exists(here(local_dir, csv1_name))) {
  # Load the uncompressed CSV, compress, and save
  mob_pro <- vroom(here(local_dir, original_csv1))
  vroom_write(mob_pro, here(local_dir, csv1_name), delim = ",")
  # Remove the original uncompressed CSV
  file.remove(here(local_dir, original_csv1))
} else {
  # Load the already-compressed CSV
  mob_pro <- vroom(here(local_dir, csv1_name))
}

# Check and process the second CSV
if (!file.exists(here(local_dir, csv2_name))) {
  # Load the uncompressed CSV, compress, and save
  var_mob_pro <- vroom(here(local_dir, original_csv2))
  vroom_write(var_mob_pro, here(local_dir, csv2_name), delim = ",")
  # Remove the original uncompressed CSV
  file.remove(here(local_dir, original_csv2))
} else {
  # Load the already-compressed CSV
...
print(knitr::kable(summary_table, format = "pipe", caption = paste("Summary for Dataset:", name))
  )
  cat("\n")
}
...
# Filter out extreme value (Paris)
filtered_no_paris_final_dataset <- final_dataset_cleaned %>%
  filter(intercommunal_population_2021 < 550000) # Set your threshold

# Histogram for intercommunal population with formatted x-axis
ggplot(filtered_no_paris_final_dataset, aes(x = intercommunal_population_2021)) +
  geom_histogram(binwidth = 500, fill = "skyblue", color = "black", alpha = 0.8) +
  labs(
    title = "Distribution of Intercommunal Population (2021), without top 8",
    x = "Intercommunal Population",
    y = "Count"
  ) +
  scale_x_continuous(labels = comma) + # Format x-axis labels as comma-separated numbers
  theme_minimal()

top8_intercommunalities <- data.frame(
  intercommunal_label = c(
  ...
  intercommunal_population_2021 = c(
    7103801,
    1911311,
    1424069,
    1187907,
...
# Render the table in knitr
print(knitr::kable(
  top8_intercommunalities,
  col.names = c("Intercommunal Label", "Population (2021)"),
  format = "pipe",
  caption = "Top Intercommunalities by Population (2021)"
  )
)
...
# Save the cleaned dataset to a CSV file
write.csv(final_dataset_cleaned2.0, "final_dataset_cleaned.csv", row.names = FALSE)
...
(EDA), visualizations, and statistical tests to assess relationships.
...
As expected, share of workers using public transport is higher when population density is higher. This seems reasonable as denser areas are better equipped with public transport. On the other hand, the lower the density, the more people use their cars to comute.
...
These bar plots show the link between the median standard of living in an intercommunauté and mode of transport used to get to work. We notice the following:
-   The two income categories that bike the most are low and high.
-   For all categories, cars, trucks and vans are by far the most common modes of transport.
-   Mothorized two-wheelers are particularly used in intercommunautés with mid-low median living standards.
-   The higher the median standard of living in an intercommunauté, the lower the average share of people not using transport.
-   Mostly 
...
Pearson Correlation: 0.3721486
Spearman Correlation: 0.4710873
...
Use chi-squared tests (chisq.test()) for dependencies between categorical variables. For instance: Urban/rural classification vs. preferred transportation mode. Socio-professional category vs. car ownership.
...
data:  urban_transport_matrix
X-squared = 2903.6, df = 5, p-value < 2.2e-16
...
Our analysis revealed several findings regarding commuting behavior in France. From the regression models, population density emerged as a significant determinant of transportation choices, with higher densities strongly correlating with increased public transport usage and reduced car dependency. Income levels also played a role, as lower-income groups showed a higher reliance on public transport and active modes like walking, while higher-income groups favored private vehicles. Additionally, gender differences were shown, with men using motorized two-wheelers more frequently and women relying slightly more on public transport. Education levels were linked to sustainable transportation choices, with higher-educated individuals showing greater shares of public transport and bicycle usage. The analysis of commuting distances also showed the practicality of certain transport modes, with public transport usage increasing for longer commutes and active modes like walking or cycling being dominant for shorter distances. 
These findings show the links among socio-demographic, economic, geographic factors, as well as commuting behaviors. While our study focuses on correlations rather than causations, it tries to identify significant trends that could inform or guide future research.

## The influence of our variables

Different factors likely play varying roles in influencing commuting behavior. For instance, population density and access to public transport infrastructure are expected to be more impactful than individual-level variables like education. To better understand the relative importance of these factors, we could apply a chi-squared (χ²) test to examine the independence and association of categorical variables (e.g., socio-professional category and transport modes).
However, we do recognize that the associations observed in this study cannot account for confounding variables, such as individual preferences, workplace policies, or social norms. A causal analysis would require further access to public data and a rigorous regression analysis.

## Modelling commuting behavior

Using regression models, we explored how variables like population density, car ownership, income levels, and public transport availability shape commuting choices. These models provide insights into how certain factors—like proximity to transport stops or urbanization—relate to increased public transport use, while others, such as higher income, correlate with reliance on private vehicles.
Modelling could be expanded to predict commuting behavior based on these variables, and could maybe be applied to policymaking, by simulating the impact of interventions like expanding public transport networks or incentivizing active modes of commuting (e.g., walking or cycling).

## The role of remote working

The COVID-19 pandemic brought large changes to commuting patterns, with remote working reducing overall commute volumes and altering transportation mode shares. It would be interesting to extend our research by comparing pre- and post-COVID commuting data to measure the long-term impact of remote working on commute behavior. This could reveal shifts in preferences and highlight the potential for permanent changes in urban mobility.

## Possible future directions

While our analysis identifies trends and relationships, our study has several limitations, such as the inability to infer causation and the potential for unmeasured confounding variables. Integrating additional data sources, such as workplace policies or real-time transport usage, could improve the robustness of our findings.
By better modeling commuting behavior and accounting for changes like remote work, we could better understand how to design policies that effectively promote sustainable and equitable transportation systems.

# Conclusion

In the Harvard Business Review article "Why It’s So Hard to Change People’s Commuting Behavior" by Ariella Kristal and Ashley Whillans, the authors emphasize how entrenched commuting habits are and the difficulty in shifting them, even when presented with sustainable or convenient alternatives. This underscores the significance of understanding the factors that drive commuting choices, as any meaningful change requires addressing deeply ingrained behaviors, environmental constraints, and individual preferences.

Our analysis highlighted patterns of commuting behavior, such as the link between population density and transportation choices. Income and education levels also emerged as influential in our findings. Moreover, gender and commuting distances revealed further differences.

While our study identified meaningful correlations, it emphasized the complexity of commuting behavior and the need for further research to account for causal relationships and confounding variables. Future work could explore the long-term impacts of remote working, policy interventions, and urban planning on commuting patterns. 

    
```{r}
write.csv(final_dataset_cleaned2.0, "final_dataset_cleaned2.0.csv", row.names = FALSE)

```

```{r}
saveRDS(mob_pro_cleaned, "mob_pro_cleaned.rds", compress = "xz")
```
