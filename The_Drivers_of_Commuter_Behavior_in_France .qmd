...
## 1. Joining local_mob_cleaned and revenus_cleaned
...
## 2. Filtering and selecting relevant columns
...
1. Put together all arondissements in Lyon, Paris, Marseille

```{r}
iltduu_sexe_trans_cleaned2.0 <- iltduu_sexe_trans_cleaned %>%
  mutate(
    communal_label = case_when(
      grepl("Lyon", communal_label, ignore.case = TRUE) ~ "Lyon",
      grepl("Marseille", communal_label, ignore.case = TRUE) ~ "Marseille",
      grepl("Paris", communal_label, ignore.case = TRUE) ~ "Paris",
      TRUE ~ communal_label
    ),
    communal_code = case_when(
      communal_label == "Lyon" ~ "69381",
      communal_label == "Marseille" ~ "13201",
      communal_label == "Paris" ~ "75001",
      TRUE ~ communal_code
    )
  ) %>%
  group_by(geographical_type, communal_code, communal_label, urban_workplace_indicator, sexe, main_transportation_work) %>%
  summarise(
...
2. Filter out geotype.

### stops_france

1. Make sure stops_france doesn't have any "doublons" left.
2. Use each stops lon/lat to associate with an "intercommune".
3. Indicate nb of stops per intercommune.
