...
## Removing the columns we won't need in mob_pro_cleaned (to be done together)
...
mob_pro_cleaned <- mob_pro_cleaned %>%
  select(-immigration_status, -nationality, -nb_students_trainee_over_14_household, -weight, -link_household_reference_person, -cohabitation_mode, -nb_employed_people_household, -nb_scolarised_people_household, ) # Replace with the actual column names
...
mob_pro_cleaned <- mob_pro_cleaned %>%
  mutate(
    communal_code = as.character(communal_code),
    # We need to add all the variables here
  )
