...
ggplot(final_dataset_cleaned, aes(x = population_density_2021, y = share_car_for_work)) +
  geom_point() +
  labs(
...
final_dataset_cleaned_2 <- final_dataset_cleaned %>%
...
melted_data <- final_dataset_cleaned_2 %>%
  select(income_category, share_worker_public_transport, share_car_for_work, share_bike_for_work) %>%
  pivot_longer(cols = starts_with("share"), names_to = "Transport_Mode", values_to = "Share")
...
```{r}
final_dataset_cleaned_3 <- final_dataset_cleaned %>%
  mutate(urban_rural = ifelse(population_density_2021 > 150, "Urban", "Rural"))

# Boxplot for Urban/Rural vs. Car Share for Work
ggplot(final_dataset_cleaned_3, aes(x = urban_rural, y = share_car_for_work)) +
  geom_boxplot() +
  labs(
